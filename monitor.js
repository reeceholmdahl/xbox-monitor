...
async function checkStock() {
    ...
    for (const [index, store] of AppState.monitorData.stores.entries()) {
        ...
        setTimeout(function() {
            fetch(url, {
                ...
                .catch(rejection => {
                    console.error(`Issue with request to store #${store.id}, ${store.zip}; ${store.name}:`)
                    console.error(rejection);
                });
        }, Math.random() * index * 250);
    }
}
